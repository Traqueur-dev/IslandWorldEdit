...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
	...
	public Location getCenterPlane() {
		int x1 = this.getUpperX() + 1;
		int z1 = this.getUpperZ() + 1;
		return new Location(this.getWorld(), this.getLowerX() + (x1 - this.getLowerX()) / 2.0, this.getLowerY(),
	 ...
	 * 
	 * @param id - The material of block to check for
	 * @return true if this Cuboid contains only blocks of the given type
	 */
	public boolean containsOnly(Material id) {
		if (id.isSolid())
			return false;
		for (Block b : this) {
			if (b.getType() != id)
